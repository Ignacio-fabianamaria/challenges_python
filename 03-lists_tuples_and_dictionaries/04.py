...
def format_phone_number(phone_number):
    ...
    if len(phone_number) == 11:
        formatted_phone_number = f'{phone_number[:2]} {phone_number[2:7]}-{phone_number[7:]}'
    ...
    while True:
        try:
            ...
            if user_option not in (0, 1, 2, 3):
                ...
                search_contact = input('Insira o nome do contato que deseja pesquisar: ').title()
                if search_contact in contacts:
                    print(f'O telefone do contato {search_contact} é : {contacts[search_contact]}')
                    break
                else:
                    print(f'Contato {search_contact} não encontrado.')
            elif user_option == 2:
                name = input('Insira o nome do novo contato: ').title()
                phone = format_phone_number(input('Insira o telefone com DDD do novo contato: '))
                contacts[name] = phone
                print(f'Novo contato adicionado à s lista.\nDetalhes: Nome: {name}, telefone: {phone}')
                break
            elif user_option == 3:
                print(contacts)
                break
